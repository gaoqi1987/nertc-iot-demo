...
static void transfer_major_task_transfer_start(uint32_t param)
{
	...
	transfer_major_task_send_msg(TRS_TRANSFER_LOG, 0);
...
static void transfer_major_task_transfer_log(uint32_t param)
{
	transfer_major_node->event = EVENT_TRANS_LOG_NOTIFY;
	transfer_major_node->param = (uint32_t)media_debug;
	//msg_send_req_to_media_major_mailbox_sync(transfer_major_node, APP_MODULE);
	msg_send_notify_to_media_major_mailbox(transfer_major_node, APP_MODULE);
}
...
static void transfer_major_task_entry(beken_thread_arg_t data)
{
	...
	rtos_set_semaphore(&transfer_major_sem);
	...
	while (1)
	{
		...
		if (kNoErr == ret)
		{
			switch (msg.type)
			{
				...
				case TRS_TRANSFER_LOG:
					transfer_major_task_transfer_log(msg.data);
					break;
...
static bk_err_t transfer_major_task_init(void)
{
	...
	media_debug->wifi_kbps = 0;
	media_debug->meantimes = 0;
	media_debug->count = 0;
	media_debug->retry_times = 0;
	...
	if (transfer_major_sem == NULL)
	{
		ret = rtos_init_semaphore(&transfer_major_sem, 1);
		if (ret != BK_OK)
		{
			LOGE("%s, transfer_major_sem init error", __func__);
			goto error;
		}
	}
	...
	rtos_get_semaphore(&transfer_major_sem, BEKEN_NEVER_TIMEOUT);
...
static bk_err_t transfer_major_task_close_handle(media_mailbox_msg_t *msg)
{
	...
	frame_buffer_fb_deregister(MODULE_WIFI, transfer_info.param);
