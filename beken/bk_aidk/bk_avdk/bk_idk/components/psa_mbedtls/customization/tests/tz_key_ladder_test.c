...
int te200_key_ladder_main(void)
{
	...
	if(ret != 0){
		os_printf("dubhe_aes_ladder_set_model_key encrypt fail: 0x%x \n", ret);
		return -1;
	}
	os_printf("key_ladder test begin 3");
#if 0
	/*encrypt*/
	ret = dubhe_aes_ladder_crypt_cbc(&ctx, 1, sizeof(key_ladder_plain_text), input_iv, key_ladder_plain_text,
                         m_encrypted_text);
	if(ret != 0){
		os_printf("dubhe_aes_ladder_crypt_cbc encrypt fail: 0x%x \n", ret);
		return -1;
	}
	/*decrypt*/
