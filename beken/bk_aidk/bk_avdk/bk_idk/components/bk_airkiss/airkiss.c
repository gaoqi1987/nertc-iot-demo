...
{ 
    int isvalid = 0; 
    u8 *mac_ptr = NULL;
    struct wifi_mac_hdr *fmac_hdr = (struct wifi_mac_hdr *)frame;
...
static int airkiss_add_src_macinfo(u8 mac_crc, u8 ap_sta, u16 f_len, u16 f_seq)
{
    ...
    if (src_num >= MAX_SRC_NUM) {
        akconf->printf("reached max src_num\r\n");
        return -1;
    }
    for(i=0; i<src_num; i++) 
    {
...
{ 
    ...
    { 
        ...
        _akcontext->seq_success_map_cmp = (1 << _akcontext->need_seq) - 1; // EXAMPLE: need_seq = 5; seq_success_map_cmp = 0x1f; ?������?? 
