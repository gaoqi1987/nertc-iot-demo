...
#if (CONFIG_SDCARD)
		uint32_t sd_card_owner = 0;
		sd_card_owner = bk_sd_card_get_owner();
		if(sd_card_owner & SD_CARD_OWNER_LOCAL_FS) {
			CLI_LOGI("%s ,line:%d,sd card is owned by fatfs\r\n",__FILE__,__LINE__);
		}
		if(sd_card_owner & SD_CARD_OWNER_USB_DEVICE) {
			CLI_LOGI("%s ,line:%d,sd card is owned by usb\r\n",__FILE__,__LINE__);
		}
		return;
#endif
