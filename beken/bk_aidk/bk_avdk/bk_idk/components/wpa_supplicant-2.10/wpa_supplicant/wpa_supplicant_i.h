...
typedef struct wapi {
	...
	void (*wapi_deauth)(struct wpa_supplicant *, u16);
	...
	void (*set_debug_level)(int); /* for wapilib debug */
	void (*wapi_process_assoc_event)(u8 *, u8 *, u8 *, u8);
	void (*wapi_process_disassoc_event)(u8 *, u8 *);
	long (*wapi_check_offset)(void);
	int (*wapi_set_hex_psk)(const char *, size_t);

	/* funciton callbacks used in lib */
	int (*wapi_set_key)(struct wpa_supplicant *wpa_s, enum wpa_alg alg,
