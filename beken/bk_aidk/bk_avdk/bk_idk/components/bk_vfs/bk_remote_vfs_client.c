...
#define VFS_OPERATE_TIMEOUT         600

static bool s_vfs_client_init = false;

static uint32_t         vfs_socket_handle = 0;
static beken_mutex_t    vfs_mutex = NULL;
static bool s_mounted_flag = false;
...
// #define VFS_SVR_TEST
#ifdef VFS_SVR_TEST
#include "vfs_svr_test.c"
#endif



bk_err_t bk_vfs_driver_init(void)
{
	if (s_vfs_client_init)
	{
		return BK_OK;
	}
	...
	int ret = rtos_init_mutex(&vfs_mutex);
	...
	vfs_socket_handle = mb_ipc_socket(IPC_GET_ID_PORT(VFS_CLIENT), NULL);
	if(vfs_socket_handle == 0)
	{
		#if LOCAL_TRACE
		BK_LOGE(TAG, "vfs-drv-client create socket failed\r\n");
		#endif
		
		goto init_fail_exit;
	}
	
	ret = mb_ipc_connect(vfs_socket_handle, IPC_GET_ID_CPU(VFS_SERVER), IPC_GET_ID_PORT(VFS_SERVER), 500);

	if(ret != 0)
	{
		#if LOCAL_TRACE
		BK_LOGE(TAG, "vfs-drv-client connect failed %d\r\n", ret);
		#endif
		
		goto init_fail_exit;
	}

	s_vfs_client_init = true;
...
init_fail_exit:
	...
	if(vfs_socket_handle != 0)
	{
		mb_ipc_close(vfs_socket_handle, VFS_OPERATE_TIMEOUT);
		vfs_socket_handle = 0;
	}
	
	rtos_deinit_mutex(&vfs_mutex);
	vfs_mutex = NULL;

	return BK_FAIL;
}

bk_err_t bk_vfs_driver_deinit(void)
{
	if(!s_vfs_client_init)
		return BK_OK;
	...
	s_vfs_client_init = false;

	return BK_OK;
}


static bk_err_t remote_vfs_mount(void)
{
	int  ret_val = BK_FAIL;
	int  line_num;
	...
	int ret = mb_ipc_send(vfs_socket_handle, VFS_CMD_MOUNT, 
...
static int bk_vfs_mount_point_init(void){
	int ret_val = BK_OK;
	...
	//When open path first time, we may auto mount to root point for application.
	ret_val = remote_vfs_mount();
	if(ret_val != BK_OK) {
		BK_LOGE(TAG, "%s mount fail @%d, ret=%d.\r\n", __FUNCTION__, __LINE__, ret_val);
		return ret_val;
	}
	s_mounted_flag = true;
...
int bk_vfs_mount(const char *source, const char *target,
	...
	ret_val = bk_vfs_mount_point_init();
	if(ret_val != BK_OK) {
		BK_LOGE(TAG, "%s mount fail @%d, ret=%d.\r\n", __FUNCTION__, __LINE__, ret_val);
	}
	return ret_val;
...
static int remote_vfs_umount(void)
{
	int  ret_val = BK_FAIL;
	int  line_num;
	...
	int ret = mb_ipc_send(vfs_socket_handle, VFS_CMD_UMOUNT, 
		(u8 *)&cmd_buff, sizeof(cmd_buff), VFS_OPERATE_TIMEOUT);
...
static int bk_vfs_mount_point_deinit(void){
	int ret_val = BK_OK;
	...
	ret_val = remote_vfs_umount();
	if(ret_val != BK_OK) {
		BK_LOGE(TAG, "%s unmount fail @%d, ret=%d.\r\n", __FUNCTION__, __LINE__, ret_val);
	}
	s_mounted_flag = false;
...
{
	bk_vfs_mount_point_deinit();

	bk_vfs_driver_deinit();
	return BK_OK;
}
...
static int remote_vfs_open(const char *path, int oflag)
{
	...
	cmd_buff.path =  os_malloc( strlen(path) + 1 );

	strncpy(cmd_buff.path, path, (strlen(path)+1));


	cmd_buff.oflag = oflag;

	rtos_lock_mutex(&vfs_mutex);

	int ret = mb_ipc_send(vfs_socket_handle, VFS_CMD_OPEN,
		(u8 *)&cmd_buff, sizeof(cmd_buff), VFS_OPERATE_TIMEOUT);
...
int bk_vfs_open(const char *path, int oflag)
{
	...
	ret_val = remote_vfs_open(path, oflag);
	if(ret_val < 0) {
		BK_LOGE(TAG, "%s open path fail @%d, ret=%d.\r\n", __FUNCTION__, __LINE__, ret_val);
	}
...
static int remote_vfs_close(int fd)
{
	int  ret_status = BK_FAIL;
	int  line_num = 0;
	...
	int ret = mb_ipc_send(vfs_socket_handle, VFS_CMD_CLOSE,
		(u8 *)&cmd_buff, sizeof(cmd_buff), VFS_OPERATE_TIMEOUT);
	...
	ret_status = cmd_buff.ret_status;
...
int bk_vfs_close(int fd)
{
	...
	ret_val = remote_vfs_close(fd);
	return ret_val;
}



static ssize_t remote_vfs_read(int fd, void *user_buf, size_t count)
{
	int  ret_read_val = 0;
	int  line_num = 0;
	
	//if(count > 0xFFFF)  //file buf count size may larger than 64K
	//	return BK_FAIL;

	if(count <= 0)  //read length 0 is invalid, just return.
		return BK_OK;
	...
	cmd_buff.fd = fd;
	cmd_buff.count  = count;

	rtos_lock_mutex(&vfs_mutex);

	int ret = mb_ipc_send(vfs_socket_handle, VFS_CMD_READ, 
			(u8 *)&cmd_buff, sizeof(cmd_buff), VFS_OPERATE_TIMEOUT);
	...
	if(cmd_buff.ret_status == 0)
	{
		line_num = __LINE__;
		ret_read_val = cmd_buff.ret_status;

		goto read_exit;
	}


	if(cmd_buff.count != count)
	{
		line_num = __LINE__;
		ret = cmd_buff.count;
	...
	#if CONFIG_CACHE_ENABLE
	flush_dcache(cmd_buff.buff, count);
	#endif
	memcpy(user_buf, cmd_buff.buff, count);

	u32  crc = calc_crc32(0, (const u8 *)user_buf, cmd_buff.ret_status);
...
#if LOCAL_TRACE
	if(ret_read_val == 0)
		BK_LOGE(TAG, "%s @%d, ret_read_val=%d.\r\n", __FUNCTION__, line_num, ret_read_val);
#endif

	ret = mb_ipc_send(vfs_socket_handle, VFS_CMD_READ_DONE, 
			(u8 *)&cmd_buff, sizeof(cmd_buff), VFS_OPERATE_TIMEOUT);
	...
	ret = mb_ipc_recv(vfs_socket_handle, &user_cmd, (u8 *)&cmd_buff, 
			sizeof(cmd_buff), VFS_OPERATE_TIMEOUT);  // it is just a handshake. every send cmd to server must have a recv.
...
ssize_t bk_vfs_read(int fd, void *buf, size_t count)
{
	...
	ret_val = remote_vfs_read(fd, buf, count);
	return ret_val;
}

static bk_err_t remote_vfs_write(int fd, const void *user_buf, size_t count)
{
	int  ret_val = BK_FAIL;
	int  line_num = 0;
	...
	cmd_buff.crc  = calc_crc32(0, user_buf, count);

	rtos_lock_mutex(&vfs_mutex);

	int ret = mb_ipc_send(vfs_socket_handle, VFS_CMD_WRITE, 
		(u8 *)&cmd_buff, sizeof(cmd_buff), VFS_OPERATE_TIMEOUT + count / 200);  // vfs write speed: write done 200 bytes in 1ms.
...
ssize_t bk_vfs_write(int fd, const void *buf, size_t count)
{
	...
	ret_val = remote_vfs_write(fd, buf, count);
