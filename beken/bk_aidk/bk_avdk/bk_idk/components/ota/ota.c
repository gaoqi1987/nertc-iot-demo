...
int bk_ota_process_data_handler (char*buff_data, uint32_t len,uint32_t received, uint32_t total)
{
	...
	if(ota_info->fota_dl_info.wr_first_flag == 0)
	{
		ota_info->fota_dl_info.image_size = total;
		ota_info->fota_dl_info.wr_first_flag = 1;
	}
	...
	switch (ota_info->ota_dest)
	{
		...
		{
			ret = f_ota_fun_ptr->data_process(&ota_info->fota_dl_info, len, f_ota_fun_ptr->wr_flash);
		}
		break;
		...
		case OTA_WR_TO_SD_CARD:
		{
			...
			f_ota_fun_ptr->data_process(&ota_info->fota_dl_info, len, f_ota_fun_ptr->write);
#endif
		}
		break;
