...
/* 
 * Maybe there is a SD-NAND in local hardware board, it connects with PC.
 * PC accesses SD-NAND with USB Host port, so should control SD-NAND/SDCARD
 * with owner.
 */
#if (CONFIG_USBD_MSC && CONFIG_USB_DEVICE)
/**
 * @brief     Set who onwes the SD-CARD.
 *
 * @return BK_OK:vote the owner with current param success;
 *         BK_FAIL:The sd_card has been owned by others.
 *                 If wants to vote with param-owner, please help clear previous owner.
 */
bk_err_t bk_sd_card_vote_owner(sd_card_owner_t owner);

/**
 * @brief     Clear the owner of SD-CARD with current param.
 *
 * @return none
 */
...
/**
 * @brief     Get all owneres of SD-CARD with return value.
 *
 * @return all owners
 */
