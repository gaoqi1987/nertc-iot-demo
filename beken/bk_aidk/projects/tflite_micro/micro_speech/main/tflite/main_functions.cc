...
TfLiteStatus LoadMicroSpeechModelAndPerformInference(
  ...
  MicroPrintf("MicroSpeech category predictions for <%s>\n", expected_label);
  for (int i = 0; i < kCategoryCount; i++) {
    category_predictions[i] =
        (tflite::GetTensorData<int8_t>(output)[i] - output_zero_point) *
        output_scale;
    MicroPrintf("  %.4f %s\n", static_cast<double>(category_predictions[i]),
                kCategoryLabels[i]);
  }
  ...
  if (strcmp(expected_label,
                                 kCategoryLabels[prediction_index]))
  {
    MicroPrintf("Fail Expected: %s, Category: %s\n", expected_label, kCategoryLabels[prediction_index]);
  }
  else
  {
    MicroPrintf("Success Expected: %s, Category: %s\n", expected_label, kCategoryLabels[prediction_index]);
  }
...
void loop() {
    ...
    MicroPrintf("################# g_silence_1000ms_audio_data #################\n START:\n");
    TestAudioSample("silence", (const int16_t*)g_silence_1000ms_audio_data,
                  g_silence_1000ms_audio_data_size / 2);

    MicroPrintf("################# g_noise_1000ms_audio_data #################\n START:\n");
    TestAudioSample("noise", (const int16_t*)g_noise_1000ms_audio_data,
                  g_noise_1000ms_audio_data_size / 2);

    MicroPrintf("################# g_no_30ms_audio_data #################\n START:\n");
    TestAudioSample("no", (const int16_t*)g_no_30ms_audio_data,
                  g_no_30ms_audio_data_size / 2);

    MicroPrintf("################# g_no_1000ms_audio_data #################\n START:\n");
    TestAudioSample("no", (const int16_t*)g_no_1000ms_audio_data,
                  g_no_1000ms_audio_data_size / 2);

    MicroPrintf("################# g_yes_30ms_audio_data #################\n START:\n");
    TestAudioSample("yes", (const int16_t*)g_yes_30ms_audio_data,
                  g_yes_30ms_audio_data_size / 2);

    MicroPrintf("################# g_yes_1000ms_audio_data #################\n START:\n");
    TestAudioSample("yes", (const int16_t*)g_yes_1000ms_audio_data,
                  g_yes_1000ms_audio_data_size / 2);
